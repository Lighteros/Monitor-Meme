...
test('returns a clear configuration error without exposing a credential', async () => {
  ...
  const response = await handler(new Request('https://example.test/api/escalation-x-feed?account=MonitorMeme_RH'));
...
test('fetches and normalizes an allowlisted account with paid reads behind CDN caching', async () => {
  ...
  assert.match(calls[0].url, /\/users\/by\/username\/MonitorMeme_RH/);
  ...
  assert.equal(body.account.handle, 'MonitorMeme_RH');
  assert.equal(body.posts[0].url, 'https://x.com/MonitorMeme_RH/status/99');
